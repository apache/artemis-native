...
public class FFMNativeHelper<Callback extends SubmitInfo> {
    ...
    private int ioGetEvents(MemorySegment aioCtx, MemorySegment events, long min,
                                      long max, MemorySegment timeout) throws Throwable {
        MemorySegment captureState = SHARED_CONTEXT.get().getStateCapture();
        // Direct syscall wrapper
        int result = (int) IO_GETEVENTS_HANDLE.invoke(
                captureState,
                aioCtx,
                min,
                max,
                events,
                (timeout == null ? MemorySegment.NULL : timeout)
        );
    ...
    public IOControl<Callback> newContext(int queueSize) {
        ...
        try {
            ...
            for(int i=0; i<queueSize; i++) {
                ...
                if(iocb.address() == 0) {
                    ...
                    throw new OutOfMemoryError(
                            String.format("Arena memory allocation failed: iocb[%d/%d]", i, queueSize));
                }
            ...
            logger.debug("Context created successfully: queueSize={}, ioContext=0x{}",
                    queueSize, Long.toHexString(ioContext.address()));
            return ioControl;
        } catch (Throwable t) {
    ...
    private void ioQueueRelease(MemorySegment ioContext) {
        ...
        try {
            ...
            if(result < 0) {
                logger.warn("io_queue_release(0x{}) failed: errno={}",
                        Long.toHexString(ioContext.address()), ERRNO_VH.get(captureState, 0L));
            } else {
                logger.trace("io_queue_release(0x{}) successful",
                        Long.toHexString(ioContext.address()));
            }
        } catch (Throwable e) {
    ...
    private MemorySegment ioQueueInit(int queueSize) {
        try(Arena arena = Arena.ofConfined()) {
            ...
            logger.trace("ioQueueInit({}) → 0x{} (result={})",
                    queueSize, Long.toHexString(rawAddress), result);
    ...
    public void deleteContext(IOControl ioControl) {
        ...
        logger.debug("deleteContext: queueSize={}, ioContext=0x{}",
                ioControl.queueSize(), Long.toHexString(ioControl.ioContext().address()));
        try {
    ...
    * */
            ...
            try(Arena arena = Arena.ofConfined()) {
                ...
                while(running) {
                    ...
                    for(int i=0; i<result; i++) {
                        ...
                        MemorySegment event = harvestedEvents
                                .asSlice(i * IO_EVENT_LAYOUT.byteSize(),
                                        IO_EVENT_LAYOUT.byteSize());
                        MemorySegment iocbp = IOEvent.getObj(event)
                                .reinterpret(IOCB_LAYOUT_SIZE);
    ...
    private static void fdatasync(Arena arena, int fd) throws Throwable {
        ...
        if(res < 0) {
            throw new IOException("fdatasync(fd = " + fd + ") failed, errno: "
                    + ERRNO_VH.get(captureState, 0L));
        }
    }
    ...
    public static boolean lock(int fd) {
        ...
        try(Arena arena = Arena.ofConfined()) {
            MemorySegment captureState = arena.allocate(CAPTURE_STATE_LAYOUT);
            int result = (int) FLOCK_HANDLE.invokeExact(captureState,
                    fd, LOCK_EX | LOCK_NB);
            return result == 0;
        } catch (Throwable t) {
    ...
    public static void fill(int fd, int alignment, long size) throws IOException {
        ...
        try {
            ...
            for (long i=0; i<blocks; i++) {
                ...
                long written = (long) WRITE_HANDLE.invoke(captureState,
                        fd, bufferAddrs, ONE_MEGA);
                if (written < 0) {
                    int errno = (int) ERRNO_VH.get(captureState, 0L);
